...
// Create new media playlist
// Add custom tags to media playlist
func TestCustomTagsForMediaPlaylist(t *testing.T) {
	expected := `#EXTM3U
#EXT-X-VERSION:3
#EXT-X-MEDIA-SEQUENCE:0
#EXT-X-TARGETDURATION:0
#EXTINF:-1.000 tvg-id="movie0.id" tvg-name="MOVIE 0" tvg-logo="movie0.jpg" group-title="MOVIE_GROUP",MOVIE 0
movie0.ts
`
	...
	tests := []struct {
		...
		{[]*CustomTag{{"tvg-id", "movie0.id"},
			{"tvg-name", "MOVIE 0"},
			{"tvg-logo", "movie0.jpg"},
			{"group-title", "MOVIE_GROUP"}}},
	}

	for _, test := range tests {
		p, e := NewMediaPlaylist(1, 3)
		if e != nil {
			t.Fatalf("Create media playlist failed: %s", e)
		}
		if e := p.Append("movie0.ts", -1, "MOVIE 0", test.Tags); e != nil {
			t.Error(e)
		}
